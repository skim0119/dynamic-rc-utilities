...
class DiscreteTemporalRCAnalysis(OperatorMixin):
    ...
    num_training_patterns: int | None = None
    ...
    def __call__(self, training_states, input_signal, *test_input_states_tuples):
        ...
        if len(training_states) == 0 and len(input_signal[0]) == 0:
            self.logger.info(
                f"missing data: training_states shape {X.shape=}, input_signal shape {y.shape=}"
            )
            return 0

        if self.num_training_patterns is not None:
            X = training_states[: self.num_training_patterns]
            y = input_signal[0][: self.num_training_patterns]
        else:
            X = training_states
            y = input_signal[0]
        ...
        if len(self.rests) > 1:
            ...
            self.logger.info("done: reconstruction statistics plot")
        else:
            self.logger.info("skip: reconstruction statistics plot")

        # ------------------- Debug and other statistics ---------
        self.tuning_curve(clf, X, y, log)
        self.statistics(X, y, log)
        self.logger.info("done: tuning plot")
        self.firing_rate_plot(X, y, log)
        self.logger.info("done: firing rate plot")
    ...
    def reconstruction_statistics_with_test(self, clf, X, y, X_tests, y_tests, log):
        ...
        precision, recall, f1, support = precision_recall_fscore_support(
            val_y, val_label, average="weighted", zero_division=0
        )
        ...
        for test_index, (X_test, y_test) in enumerate(zip(X_tests, y_tests)):
            ...
            cmatrix = confusion_matrix(
                y_test, test_predicted_label, labels=patterns, normalize="true"
            )
            ...
            save_path = os.path.join(
                self.analysis_path, f"reconstruction_after_rest_{rest}.png"
            )
            ...
            precision, recall, f1, support = precision_recall_fscore_support(
                y_test, test_predicted_label, average="weighted", zero_division=0
            )
        ...
        plt.close()
        np.savez(
            os.path.join(self.analysis_path, "reconstruction_test.npz"),
            rests=self.rests,
            f1s=f1s,
        )
        ...
        for tasks in np.array_split(indices, 100):
            score = f1_score(
                y[indices[tasks]], clf.predict(X[indices[tasks]]), average="weighted"
            )
            scores.append(score)
        plt.figure()
        plt.plot(scores)
        ...
        plt.savefig(
            os.path.join(self.analysis_path, "reconstruction_test_over_time.png")
        )
        plt.close()
    ...
    def tuning_curve(self, clf, X, y, log):
        ...
        size, train_scores, test_scores = learning_curve(
            clf, X, y, train_sizes=np.linspace(0.1, 1.0, 20), cv=10
        )
        plt.figure()
    ...
    def firing_rate_plot(self, X, y, log):
        ...
        for pattern in patterns:
            ...
            plt.savefig(
                os.path.join(
                    self.analysis_path, f"mean_firing_rate_per_pattern_{pattern}.png"
                )
            )
            plt.close()
